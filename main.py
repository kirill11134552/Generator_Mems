from PIL import Image, ImageDraw, ImageFont


print("Генератор мемов запущен!")
#Ввод текстов
top_text = input("Напиши верхний текст мема: ")
bottom_text = input("Напиши нижний текст мема: ")
print(top_text, bottom_text)

#Вывод список картинок
print("Список картинок:")
print("1. Кот в ресторане")
print("2. Кот в очках")

#Выбор картинки
image_number = input("Введи номер картинки: ")
if image_number == "1":
    image_name = "Кот в ресторане.png"
elif image_number == "2":
    image_name = "Кот в очках.png"

#Работа с картинкой
image = Image.open(image_name)
width, height = image.size
draw = ImageDraw.Draw(image)
font = ImageFont.truetype('arial.ttf', size=70)

#Отрисовка верхнего текста
text = draw.textbbox((0, 0), top_text, font)
text_width = text[2]
draw.text(((width - text_width) / 2, 10), top_text, font=font, fill='black')

#Отрисовка нижнего текста
text = draw.textbbox((0, 0), bottom_text, font)
text_width = text[2]
text_height = text[3]
draw.text(((width - text_width) / 2, height - text_height - 10), bottom_text, font=font, fill='black')

#Сохранения картинки
new_name = input("Введите название вашей картинки:")

image.save(new_name + ".jpg")
